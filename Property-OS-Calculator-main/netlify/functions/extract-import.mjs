...
export async function handler(event) {
  ...
  try {
    ...
    const outputText = await runExtraction(body, provider);
...
async function runExtraction(body, provider) {
  if (provider === "ollama") {
    return runOllamaExtraction(body);
  }

  return runOpenAiExtraction(body);
}

async function runOpenAiExtraction(body) {
  ...
  if (!response.ok) {
    return Promise.reject(
      new Error(data.error?.message || "OpenAI extraction failed.")
    );
  }
...
async function runOllamaExtraction(body) {
  ...
  const baseUrl = String(process.env.OLLAMA_BASE_URL || "http://127.0.0.1:11434/api").replace(/\/$/, "");
...
function getExtractionProvider() {
  return String(process.env.EXTRACTION_PROVIDER || DEFAULT_EXTRACTION_PROVIDER)
    .trim()
    .toLowerCase();
}
...
function validateProviderConfig(provider) {
  ...
  return "OPENAI_API_KEY is not set in Netlify environment variables.";
}
